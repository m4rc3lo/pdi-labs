...
## Galeria de entradas oficiais

O par abaixo demonstra o padrão visual reutilizável nas demais páginas. Clique
na miniatura para abrir a imagem original. O caso A favorece a observação de
canais e quantização; o caso B introduz maior diversidade de formas e detalhes.

<div class="pdi_gallery_grid">
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/input/ai_realistic/m1_1/m1_1_a_rgb_quantizacao.png"><img src="../images/input/ai_realistic/m1_1/m1_1_a_rgb_quantizacao.png" alt="Entrada A do Laboratório M1.1 para canais, níveis de cinza e quantização." loading="lazy"></a>
<div class="pdi_gallery_caption"><strong>Caso A — favorável.</strong> Objetos coloridos e fundo controlado para comparar canais e conversões.</div>
</div>
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/input/ai_realistic/m1_1/m1_1_b_rgb_quantizacao.png"><img src="../images/input/ai_realistic/m1_1/m1_1_b_rgb_quantizacao.png" alt="Entrada B do Laboratório M1.1 com maior complexidade visual." loading="lazy"></a>
<div class="pdi_gallery_caption"><strong>Caso B — desafiador.</strong> Maior variedade de detalhes para avaliar perda tonal após quantização.</div>
</div>
</div>
...
## 13. Quantização manual em níveis de cinza

A classe `pdi::value::GrayscaleQuantizer` reduz uma imagem `CV_8UC1` para uma
quantidade finita de intensidades. Este incremento suporta:

```text
2, 4, 8, 16 e 256 níveis
```

### 13.1 Seleção do intervalo

Para uma intensidade original `v` e uma quantidade `L` de níveis, o índice do
intervalo é:
...
A multiplicação é feita antes da divisão. O cálculo utiliza valores inteiros
com capacidade suficiente para evitar overflow e impedir uma divisão inteira
prematura como `L / 256`.

### 13.2 Política de reconstrução

O projeto adota reconstrução uniforme preservando os extremos `0` e `255`:

```text
q(k) = round(k × 255 / (L - 1))
```

Com quatro níveis, os valores reconstruídos são:

```text
0, 85, 170 e 255
```

A política separa duas decisões:

1. o valor original determina o intervalo uniforme ao qual pertence;
2. o índice do intervalo é convertido para uma intensidade uniformemente
   distribuída entre os extremos.

Para `L = 256`, a transformação preserva todos os valores originais.

### 13.3 Exemplos de fronteira
...
flowchart TD
    ...
    Validate["Validar imagem e quantidade de níveis"]
    Traverse["Percorrer linhas e colunas"]
    Bin["Calcular k = floor(v L / 256)"]
    Reconstruct["Reconstruir round(k 255 / (L - 1))"]
    Output["Saída CV_8UC1"]

    Input --> Validate --> Traverse --> Bin --> Reconstruct --> Output
    classDef default fill:#dff3fa,stroke:#0786b4,color:#17252d,stroke-width:1.5px;
    linkStyle default stroke:#045c7d,stroke-width:1.5px;
...
```text
função quantizar(imagem, níveis):
    validar imagem CV_8UC1
    validar potência de dois no intervalo [2, 256]
    validar níveis suportados: 2, 4, 8, 16 ou 256

    criar saída CV_8UC1

    para cada linha:
        obter ponteiro uint8_t da entrada
        obter ponteiro uint8_t da saída

        para cada coluna:
            v <- entrada[coluna]
            k <- piso(v * níveis / 256)
            reconstruído <- arredondar(k * 255 / (níveis - 1))
            saída[coluna] <- reconstruído

    retornar saída
```

### 13.6 Exemplo executável

Sem interface gráfica:

```bash
./build/ucrt64-debug/lab_m1_1_quantization.exe \
    images/input/sample_grayscale.png \
    images/output/m1_1_quantization
```

Com exibição da entrada e dos cinco resultados:
...
quantized_256_levels.png
```

Sem `--show`, o executável apenas carrega, processa, persiste e encerra. Isso
mantém o exemplo adequado para ambientes headless e integração contínua.


## 14. Integração final do Laboratório M1.1

O executável `lab_m1_1` reúne todos os componentes desenvolvidos nos
incrementos anteriores. Uma única imagem BGR gera:

```text
inspeção -> relatório CSV
cópia manual
canais B, G e R
cinza por média simples
cinza por média ponderada
quantizações de 16, 8, 4 e 2 níveis
```

A quantização integrada usa a imagem ponderada como entrada.
...
flowchart TD
    ...
    Copy["ImageCopy"]
    Channels["ChannelSeparator"]
    Gray["GrayscaleConverter"]
    Quantize["GrayscaleQuantizer"]
    Persist["pdi::io::ImageFileStorage"]
    Report["inspection_report.csv"]
    Display["ImageDisplay opcional"]

    Input --> Inspect --> Report
    Input --> Copy --> Persist
    Input --> Channels --> Persist
    Input --> Gray --> Persist
    Gray -->|ponderada| Quantize --> Persist
    Input -. --show .-> Display
    Copy -. --show .-> Display
    Channels -. --show .-> Display
    Gray -. --show .-> Display
    Quantize -. --show .-> Display
    classDef default fill:#dff3fa,stroke:#0786b4,color:#17252d,stroke-width:1.5px;
    linkStyle default stroke:#045c7d,stroke-width:1.5px;
...
O README específico está em `apps/lab_m1_1/README.md`.

## Resultados processados curados

Os exemplos desta seção foram selecionados manualmente entre as saídas reais do
executável integrado. Eles não representam todo o conteúdo de `images/output/`.
A entrada principal é a imagem realista gerada por IA
`m1_1_a_rgb_quantizacao.png`, escolhida pela diversidade cromática dos objetos.
O caso B foi mantido como exemplo desafiador de quantização.

Comando principal:

```bash
./build/ucrt64-debug/lab_m1_1.exe \
    images/input/ai_realistic/m1_1/m1_1_a_rgb_quantizacao.png \
    images/output/prompt_33_4/m1_1/favoravel
```
...
<div class="pdi_gallery_grid">
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/results/m1_1/favoravel_channel_blue.png">
<img src="../images/results/m1_1/favoravel_channel_blue.png" alt="Canal azul em níveis de cinza da entrada favorável do M1.1." loading="lazy">
</a>
<div class="pdi_gallery_caption">Canal B: objetos azuis aparecem com maior intensidade.</div>
</div>
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/results/m1_1/favoravel_channel_green.png">
<img src="../images/results/m1_1/favoravel_channel_green.png" alt="Canal verde em níveis de cinza da entrada favorável do M1.1." loading="lazy">
</a>
<div class="pdi_gallery_caption">Canal G: evidencia principalmente a pera e elementos verdes.</div>
</div>
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/results/m1_1/favoravel_channel_red.png">
<img src="../images/results/m1_1/favoravel_channel_red.png" alt="Canal vermelho em níveis de cinza da entrada favorável do M1.1." loading="lazy">
</a>
<div class="pdi_gallery_caption">Canal R: evidencia a maçã, o limão e regiões avermelhadas.</div>
</div>
</div>
...
<div class="pdi_gallery_grid">
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/results/m1_1/favoravel_grayscale_average.png">
<img src="../images/results/m1_1/favoravel_grayscale_average.png" alt="Conversão por média aritmética dos canais BGR." loading="lazy">
</a>
<div class="pdi_gallery_caption">Média simples: todos os canais contribuem igualmente.</div>
</div>
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/results/m1_1/favoravel_grayscale_weighted.png">
<img src="../images/results/m1_1/favoravel_grayscale_weighted.png" alt="Conversão ponderada para níveis de cinza." loading="lazy">
</a>
<div class="pdi_gallery_caption">Média ponderada: maior contribuição do verde e menor do azul.</div>
</div>
</div>
...
<div class="pdi_gallery_grid">
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/results/m1_1/favoravel_quantized_008_levels.png">
<img src="../images/results/m1_1/favoravel_quantized_008_levels.png" alt="Imagem favorável quantizada para oito níveis." loading="lazy">
</a>
<div class="pdi_gallery_caption">Oito níveis preservam a estrutura geral e parte das transições tonais.</div>
</div>
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/results/m1_1/favoravel_quantized_002_levels.png">
<img src="../images/results/m1_1/favoravel_quantized_002_levels.png" alt="Imagem favorável quantizada para dois níveis." loading="lazy">
</a>
<div class="pdi_gallery_caption">Dois níveis produzem perda acentuada de forma e textura.</div>
</div>
<div class="pdi_gallery_card">
<a class="pdi_gallery_link" href="../images/results/m1_1/desafiador_quantized_004_levels.png">
<img src="../images/results/m1_1/desafiador_quantized_004_levels.png" alt="Caso desafiador do M1.1 quantizado para quatro níveis." loading="lazy">
</a>
<div class="pdi_gallery_caption">Quatro níveis no caso desafiador: sombras e superfícies curvas formam regiões abruptas.</div>
</div>
</div>

A fotografia própria `foto_passaro_comedouro_pb_01.png` permanece catalogada
como **entrada originalmente em preto e branco**. Ela não é apresentada como
saída do conversor. Consulte as
